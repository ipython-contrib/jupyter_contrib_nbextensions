# -*- coding: utf-8 -*-
"""These pre and postprocessor preserve highlights when converting the notebook to html or LaTeX
Preprocessor:
- for html conversion: the preprocessor replaces html tags by a "neutral" text versions, which enables markdown conversion of text included in the data field
e.g <span class="mark"> *text* </span> is translated into !oph!span class="mark"!clh! *text* !oph!/span!clh!
- for LaTeX conversion:  the preprocessor replaces html tags by a "neutral" text versions of some associated LaTeX commands or environments. This, again enables markdown conversion of text included in the command or environment. 
e.g <span class="mark"> *text* </span> is translated into !sl!highlighta!op! *text* !cl!
Postprocessor:
- replaces the "neutral" text versions by the destination language tags
eg. [html example] becomes <span class="mark"> <em>text</em> </span>  (the data text have been correctly emphasized)
...
class HighlighterPreprocessor(Preprocessor):
    ...
    def latex_scheme_cell(self,match):
        schemes={"mark":"highlightA","burk":"highlightB","girk":"highlightC"}
        return "!sl!begin!op!"+schemes[match.group(1)]+'!cl!\n'+match.group(2)+"\n!sl!end!op!"+schemes[match.group(1)]+'!cl!\n'
        #return "\\begin{"+schemes[match.group(1)]+'}\n'+match.group(2)+"\n\\end{" +schemes[match.group(1)]+'}\n'

    def latex_scheme(self,match):
        schemes={"mark":r"!sl!highlighta","burk":r"!sl!highlightb","girk":r"!sl!highlightc"}
        #schemes={"mark":"\\highlighta","burk":"\\highlightb","girk":"\\highlightc"}
    ...
    def replace_highlights_with_latex(self, cell_text):
        cell_text=re.sub("^<div class=\"(mark|burk|girk)\">([\S\s]*?)<\/div><i class=\"fa fa-lightbulb-o \"></i>",
                    self.latex_scheme_cell,cell_text)
        cell_text=re.sub("<span class=\"(mark|burk|girk)\">([\S\s]*?)<\/span>",self.latex_scheme,cell_text)
        return cell_text

    def replace_highlights_in_html(self, cell_text):
        cell_text=re.sub("^<div class=\"(mark|burk|girk)\">([\S\s]*?)<\/div><i class=\"fa fa-lightbulb-o \"></i>",
                    self.html_replacements,cell_text)
        cell_text=re.sub("<span class=\"(mark|burk|girk)\">([\S\s]*?)<\/span>",self.html_replacements,cell_text)
        return cell_text
