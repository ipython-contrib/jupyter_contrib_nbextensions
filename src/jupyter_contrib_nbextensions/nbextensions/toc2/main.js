...
define(["require", 
    ...
    function read_config(cfg, callback) { // read after nb is loaded
        ...
        config.loaded.then(function() {
            ...
            // and moveMenuLeft, threshold, navigate_menu, wideNotebook, collapse_to_match_collapsible_headings taken globally (if it exists, otherwise default)
            //
            var takeValueFromSystem = ['moveMenuLeft', 'threshold', 'navigate_menu', 'markTocItemOnScroll', 
                'widenNotebook', 'collapse_to_match_collapsible_headings']
            takeValueFromSystem.forEach(function(item) {
                cfg[item] = IPython.notebook.metadata.toc[item] = initial_cfg[item];
                if (config.data.toc2) {
                    if (typeof config.data.toc2[item] !== "undefined") {
                        cfg[item] = IPython.notebook.metadata.toc[item] = config.data.toc2[item];
                    }
                }
            })            
